...
const Sidebar = ({ collapsed, onToggleCollapse, activeIndicators, onToggleIndicator }) => {
  const [expandedTabs, setExpandedTabs] = useState({ indicators: true })
    ...
    <div className="w-80 bg-trading-darker border-r border-trading-border flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto trading-scrollbar">
        {tabs.map(tab => {
          ...
          return (
            <div key={tab.id} className="border-b border-trading-border">
              {/* Tab Header */}
              <button
                onClick={() => toggleTab(tab.id)}
                className="w-full flex items-center justify-between p-4 text-left hover:bg-trading-border transition-colors"
              >
                <div className="flex items-center space-x-3">
                  <Icon className="w-5 h-5 text-trading-blue" />
                  <span className="text-sm font-medium text-trading-text">{tab.label}</span>
                </div>
                {isExpanded ? (
                  <ChevronUp className="w-4 h-4 text-trading-text-dim" />
                ) : (
                  <ChevronDown className="w-4 h-4 text-trading-text-dim" />
                )}
              </button>
                      ...
                      <div className="space-y-2">
                        {[
                          ...
                          return (
                            ...
                            >
                              <ItemIcon className="w-5 h-5 text-trading-blue flex-shrink-0" />
                              <div className="flex-1">
